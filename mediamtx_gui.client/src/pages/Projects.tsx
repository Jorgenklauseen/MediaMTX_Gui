...
function Projects() {
  ...
  const [regeneratingStreamId, setRegeneratingStreamId] = useState<
    string | null
  >(null);
  const [togglingRecordingStreamId, setTogglingRecordingStreamId] = useState<
    string | null
  >(null);
  ...
  const handleToggleRecording = async (projectId: number, streamId: string, enabled: boolean) => {
    try {
      ...
      const updatedStream = await toggleStreamRecording(projectId, streamId, enabled);
      setProjectStreams((current) => ({
        ...current,
        [projectId]: (current[projectId] ?? []).map((s) =>
      ...
      setStreamErrors((current) => ({
        ...current,
        [projectId]: "Could not toggle recording.",
      }));
    } finally {
                            ...
                            <div className="project-stream-actions">
                              {stream.canRotateKey && (
                                <button
                                  ...
                                  onClick={() => void handleToggleRecording(project.id, stream.id, !stream.recordingEnabled)}
                                  disabled={togglingRecordingStreamId === stream.id}
                                  title={stream.recordingEnabled ? "Recording enabled — click to disable" : "Recording disabled — click to enable"}
                                >
                                  {togglingRecordingStreamId === stream.id
                                    ? "Saving..."
